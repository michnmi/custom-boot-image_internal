pipeline {
    ...
    stages {
        ...
        stage('Chenck notifications') {
            steps {
                githubNotify account: 'michnmi', context: '', credentialsId: 'Github credentials', description: '', gitApiUrl: '', repo: 'custom-boot-image_internal', sha: "$env.GIT_COMMIT", status: 'PENDING', targetUrl: "$env.JENKINS_URL"
            }
        }
    }
