        ...
        stage('Send qcow over to vmhosts') {
            steps {
                withCredentials([
                    ...
                    sh 'rsync -a  --rsync-path="sudo rsync"  -e "ssh -o StrictHostKeyChecking=no -i ssh_keys/id_ed25519_jenkins" output-ubuntu18.04_baseos/ubuntu18.04_baseos.qcow2 $JENKINS_USER_NAME@vmhost01:/vmhost_qcow/boot/ubuntu18.04_baseos_latest.qcow2  --progress'
                    sh 'ssh -o StrictHostKeyChecking=no -i ssh_keys/id_ed25519_jenkins $JENKINS_USER_NAME@vmhost01 "sudo chown libvirt-qemu:kvm /vmhost_qcow/boot/ubuntu18.04_baseos_latest.qcow2"'
                    sh 'ssh -o StrictHostKeyChecking=no -i ssh_keys/id_ed25519_jenkins $JENKINS_USER_NAME@vmhost01 "sudo chmod 660 /vmhost_qcow/boot/ubuntu18.04_baseos_latest.qcow2"'
        ...
        stage('Update image to be used - vmhost01.') {
            steps {
                withCredentials([
                    ...
                    sh 'chmod 600 ssh_keys/id_ed25519_jenkins'
                    sh 'sed -i -e \'/^$/d\' ssh_keys/id_ed25519_jenkins'
                    sh 'ssh -o StrictHostKeyChecking=no -i ssh_keys/id_ed25519_jenkins $JENKINS_USER_NAME@vmhost01 "sudo mv /vmhost_qcow/boot/ubuntu18.04_baseos.qcow2 /vmhost_qcow/boot/ubuntu18.04_baseos_previous.qcow2"'
                    sh 'ssh -o StrictHostKeyChecking=no -i ssh_keys/id_ed25519_jenkins $JENKINS_USER_NAME@vmhost01 "sudo cp -p /vmhost_qcow/boot/ubuntu18.04_baseos_latest.qcow2 /vmhost_qcow/boot/ubuntu18.04_baseos.qcow2"'
